...
ブラウザ完結型の OCR Web アプリです。画像 / PDF を読み込み、表示中ページの上で矩形を複数選択し、選択した範囲ごとに日本語 OCR を実行できます。

公開先: https://wadatch.github.io/browser-ocr-cropper/

## 売り: 処理データはクラウドにアップロードされません

このアプリの最大の特徴は、**読み込んだ画像 / PDF がブラウザの外に出ない (サーバーに一切送信されない)** ことです。OCR 処理もすべてあなたのブラウザの中で完結します。社外秘の資料や個人情報を含むファイルでも、外部のクラウド OCR サービスに送信したくない場面で安心してお使いいただけます。

- 画像 / PDF はブラウザのメモリ上だけで扱う
- OCR は WebAssembly 版 Tesseract がブラウザ内で実行
- 外部通信は初回のみ。Tesseract の学習データ (`jpn` / `jpn_vert`) と pdf.js worker を CDN から取得するだけで、**ファイル本体の送信は発生しません**
- サーバー側のストレージ / DB / アカウント機能なし
